...
namespace dataflow_use_case_test {
    ...
    bool DataflowTest::Test() {
        ...
        // data source
        DataCallback data_callback = std::bind(&DataflowNetwork::NewData, network_, std::placeholders::_1, std::placeholders::_2);
        ...
        auto invalid_callback = std::bind(&DataSink::ReceiveInvalid, &data_sink, std::placeholders::_1,std::placeholders::_2,std::placeholders::_3);
        ...
        bool all_ok = true;
        std::vector<std::size_t> expected_valid_count;
        std::vector<std::size_t> expected_invalid_count;
        ...
        std::size_t total_valid_results = 0;
        std::size_t total_invalid_results = 0;
        for (std::size_t time_domain_idx = 0; time_domain_idx < expected_results.size();++time_domain_idx) {
            ...
            for (const auto& expected : expected_time_domain) {
                ...
                if(expected_valid) {
                    ...
                    if(in_result == result_time_domain.end()){
                        spdlog::error("Expected valid result for time domain {0}, port {1}, ts {2}, no found", time_domain_idx, expected_port, expected_ts.time_since_epoch().count());
                        all_ok = false;
                        continue;
                    }
                    if(in_invalid != invalid_time_domain.end()){
                        spdlog::error("Expected valid result for time domain {0}, port {1}, ts {2}, but found in invalid results", time_domain_idx, expected_port, expected_ts.time_since_epoch().count());
                        all_ok = false;
                        continue;
                    }
                    ...
                    if(port_result == in_result->second.end()){
                        spdlog::error("Expected valid result for time domain {0}, port {1}, ts {2}, but port not found in valid results", time_domain_idx, expected_port, expected_ts.time_since_epoch().count());
                        all_ok = false;
                        continue;
                    }
                    ...
                    if(!port_result->second.isApprox(expected_value)) {
                        ...
                        spdlog::error("Expected valid result for time domain {0}, port {1}, ts {2}, values differ, expected:\n{3}\nresult:\n{4}", time_domain_idx, expected_port, expected_ts.time_since_epoch().count(), ss_expected.str(), ss_result.str());
                        all_ok = false;
                        continue;
                    }
                    valid_results++;
                    ...
                    if(in_result != result_time_domain.end()){
                        spdlog::error("Expected invalid result for time domain {0}, port {1}, ts {2}, but found in valid results", time_domain_idx, expected_port, expected_ts.time_since_epoch().count());
                        all_ok = false;
                        continue;
                    }
                    if(in_invalid == invalid_time_domain.end()){
                    ...
                    if(port_result == in_invalid->second.end()){
                        spdlog::error("Expected invalid result for time domain {0}, port {1}, ts {2}, but port not found in invalid results", time_domain_idx, expected_port, expected_ts.time_since_epoch().count());
                        all_ok = false;
                        continue;
                    }
                    invalid_results++;
        ...
        ss << fmt::format("Total Result Events: {4} : Valid Results {0}/{1} ; Invalid Results {2}/{3}\n", valid_results,  total_valid_results,invalid_results, total_invalid_results, total_events);
        ...
        for(const auto& td_port_delay : delays) {
            ...
            std::transform(delay_data.begin(), delay_data.end(), diff_delay.begin(), [mean_delay](TimeDurationType x) { return nanoToMicro(x).count() - mean_delay.count(); });
