...
namespace dataflow_use_case_test {
    ...
    DataSink::DataSink(std::size_t time_domain_count, std::size_t expected_result_count) {
        valid_events_.resize(time_domain_count);
        invalid_events_.resize(time_domain_count);
        for(std::size_t i=0;i< time_domain_count;++i){
    ...
    void DataSink::Receive(int time_domain, PortTsData data) {
        ...
        DataTsToReceiveTs.emplace(std::make_pair(std::make_tuple(time_domain, std::get<0>(data), std::get<1>(data)), steady_now()));
    }
    ...
    std::vector<std::map<TimestampType, std::map<int, TestDataType> > > DataSink::GetDataEvents() {
        ...
        for(std::size_t time_domain=0; time_domain < valid_events_.size(); ++time_domain){
            for(std::size_t index=0; index < valid_events_[time_domain].size(); ++index){
    ...
    std::vector<std::map<TimestampType, std::set<int> > > DataSink::GetInvalidEvents() {
        ...
        for(std::size_t time_domain=0; time_domain < invalid_events_.size(); ++time_domain){
            for(std::size_t index=0; index < invalid_events_[time_domain].size(); ++index){
