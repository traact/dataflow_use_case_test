...
namespace dataflow_use_case_test {
    ...
    AllEvents_PerfectTimestamps::CreateSources(UseCaseProblem problem, bool simulate_sensors, std::size_t num_events) {
        ...
        switch (problem) {
            default:
                throw std::runtime_error("AllEvents_PerfectTimestamps unsupported UseCaseProblem");
            case UseCaseProblem::In0In1TD0_In2In3TTD1_Out0TD0_Out1TD0_Buffer: {
                config.sin_offset = 3;
                config.start_time =  TimestampType (std::chrono::nanoseconds (3000000000000000ll) + std::chrono::milliseconds(5));
    ...
    BaseProblemList AllEvents_PerfectTimestamps::CreateBaseProblems(UseCaseProblem problem, bool do_busy_work) {
        ...
        switch (problem) {
            ...
            case UseCaseProblem::In0In1_In2In3_Out0_Out1:
            case UseCaseProblem::In0In1_In2In3_Out0:
                work_delays = {std::chrono::milliseconds(5),std::chrono::milliseconds(4),std::chrono::milliseconds(8)};
                result.emplace_back(std::make_shared<BaseProblem>(do_busy_work, work_delays, expected_frequency));
            case UseCaseProblem::In0In1_In2_Out0:
                work_delays = {std::chrono::milliseconds(2),std::chrono::milliseconds(1),std::chrono::milliseconds(5)};
                result.emplace_back(std::make_shared<BaseProblem>(do_busy_work, work_delays, expected_frequency));
            case UseCaseProblem::In0In1_Out0:
                work_delays = {std::chrono::milliseconds(8),std::chrono::milliseconds(6),std::chrono::milliseconds(7)};
                result.emplace_back(std::make_shared<BaseProblem>(do_busy_work, work_delays, expected_frequency));
            case UseCaseProblem::In0_Out0:
                break;
        }
