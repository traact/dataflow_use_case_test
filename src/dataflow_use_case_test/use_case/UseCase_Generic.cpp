...
UseCase_Generic::UseCase_Generic(std::unique_ptr<UseCase_Behaviour> &&behaviour)
    : UseCase(), behaviour_(std::move(behaviour)) {
...
void UseCase_Generic::Init(std::vector<ExpectedSource> expectedSources, BaseProblemList baseProblems) {
    ...
    for (auto &ts_td : clean_timestamps_) {
        ...
        std::map<bool, std::map<size_t, TimestampType >> event_ts;

        for (size_t source_idx = 0; source_idx < expected_sources_.size(); ++source_idx) {
...
void
UseCase_Generic::CreateDataEvents(const std::map<bool, std::map<size_t, TimestampType>> &source_data,
                                  const TS_TD &ts_td) {
...
void UseCase_Generic::CreateData(const UseCase::TS_TD &ts_td, const TimeDurationType &delay,
    ...
    for (const auto &source_idx_ts : data) {
        ...
        event.Data.emplace_back(std::make_tuple(port,
                                                source.GetTimestamp(ts_td.first).value(),
                                                source.GetPose(ts_td.first)));
    }
