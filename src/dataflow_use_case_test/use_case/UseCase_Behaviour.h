...
class DATAFLOW_USE_CASE_TEST_EXPORT UseCase_Behaviour {
 public:
    ...
    virtual void CheckInputs(const std::vector<ExpectedSource> &expectedSources,
                             const BaseProblemList &baseProblems) const = 0;
    virtual void AssignExpectedSources(std::map<size_t, std::pair<int, int>> &source_idx_to_time_domain) const = 0;
    virtual void CreateResult(const std::pair<TimestampType, int> &ts_td,
                              const std::vector<ExpectedSource> &sources,
                              std::vector<std::vector<std::tuple<bool,
                                                                 int,
                                                                 TimestampType,
                                                                 TestDataType >>> &result_data) = 0;
};
}
